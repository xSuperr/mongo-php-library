...
class StaticFunctions
{
     ...
     * no effect.
     */
    public static function add_logger(\Psr\Log\LoggerInterface $logger): void
    {
     ...
     * effect.
     */
    public static function remove_logger(\Psr\Log\LoggerInterface $logger): void
    {
     ...
     */
    public static function object(mixed ...$values): stdClass
    {
     ...
     *
     * @param \MongoDB\Driver\Server $servers
     * @internal
     */
    public static function all_servers_support_write_stage_on_secondary(array $servers): bool
    {
        ...
        foreach ($servers as $server) {
            ...
            if ($server->getType() === \MongoDB\Driver\Server::TYPE_LOAD_BALANCER) {
                continue;
            }

            if (!self::server_supports_feature($server, $wireVersionForWriteStageOnSecondary)) {
                return false;
            }
        }
     ...
     * @internal
     */
    public static function apply_type_map_to_document(array|object $document, array $typeMap): array|object
    {
        if (!self::is_document($document)) {
            throw \MongoDB\Exception\InvalidArgumentException::expectedDocumentType('$document', $document);
        }

        return \MongoDB\BSON\Document::fromPHP($document)->toPHP($typeMap);
    }
     ...
     *
     * @throws \MongoDB\Exception\InvalidArgumentException if $document is not an array or object
     * @internal
     */
    public static function document_to_array(array|object $document): array
    {
        if ($document instanceof \MongoDB\BSON\Document || $document instanceof \MongoDB\BSON\PackedArray) {
        ...
        } elseif ($document instanceof \MongoDB\BSON\Serializable) {
     ...
     */
    public static function get_encrypted_fields_from_driver(string $databaseName, string $collectionName, \MongoDB\Driver\Manager $manager): array|object|null
    {
     ...
     * @see Database::dropCollection()
     */
    public static function get_encrypted_fields_from_server(string $databaseName, string $collectionName, \MongoDB\Driver\Manager $manager, \MongoDB\Driver\Server $server): array|object|null
    {
        ...
        $collectionInfoIterator = (new \MongoDB\Operation\ListCollections($databaseName, ['filter' => ['name' => $collectionName]]))->execute($server);
     ...
     * @internal
     */
    public static function is_document(mixed $document): bool
    {
        return is_array($document) || (is_object($document) && !$document instanceof \MongoDB\BSON\PackedArray);
    }
     ...
     * @internal
     */
    public static function is_first_key_operator(array|object $document): bool
    {
        if ($document instanceof \MongoDB\BSON\PackedArray) {
            return false;
        }

        $document = self::document_to_array($document);
     ...
     * @internal
     */
    public static function is_pipeline(array|object $pipeline, bool $allowEmpty = false): bool
    {
        if ($pipeline instanceof \MongoDB\BSON\PackedArray) {
            /* Nested documents and arrays are intentionally left as BSON. We avoid
        ...
        } elseif ($pipeline instanceof \MongoDB\BSON\Serializable) {
        ...
        foreach ($pipeline as $stage) {
            if (!self::is_document($stage)) {
                return false;
            }

            if (!self::is_first_key_operator($stage)) {
                return false;
            }
        }
     ...
     * @internal
     */
    public static function is_builder_pipeline(array $pipeline): bool
    {
        ...
        foreach ($pipeline as $stage) {
            if (is_object($stage) && $stage instanceof \MongoDB\Builder\Type\StageInterface) {
                return true;
            }
        }
     ...
     * @internal
     */
    public static function is_in_transaction(array $options): bool
    {
     ...
     * @internal
     */
    public static function is_last_pipeline_operator_write(array $pipeline): bool
    {
        $lastOp = end($pipeline);
        ...
        $key = array_key_first(self::document_to_array($lastOp));
     ...
     */
    public static function is_write_concern_acknowledged(\MongoDB\Driver\WriteConcern $writeConcern): bool
    {
     ...
     *
     * @param \MongoDB\Driver\Server $server Server to check
     * @param integer $feature Feature constant (i.e. wire protocol version)
     * @internal
     */
    public static function server_supports_feature(\MongoDB\Driver\Server $server, int $feature): bool
    {
        $info = $server->getInfo();
     ...
     * @internal
     */
    public static function is_string_array(mixed $input): bool
    {
        if (!is_array($input)) {
            return false;
        }
     ...
     */
    public static function recursive_copy(mixed $element): mixed
    {
        if (is_array($element)) {
            foreach ($element as $key => $value) {
                $element[$key] = self::recursive_copy($value);
            }
     ...
     * @internal
     */
    public static function create_field_path_type_map(array $typeMap, string $fieldPath): array
    {
     ...
     *
     * @param \MongoDB\Driver\Session $session A session object as retrieved by Client::startSession
     ...
     * @throws \MongoDB\Exception\RuntimeException for driver errors while committing the transaction
     ...
     */
    public static function with_transaction(\MongoDB\Driver\Session $session, callable $callback, array $transactionOptions = []): void
    {
        $operation = new \MongoDB\Operation\WithTransaction($callback, $transactionOptions);
     ...
     * @internal
     */
    public static function extract_session_from_options(array $options): ?\MongoDB\Driver\Session
    {
        if (isset($options['session']) && $options['session'] instanceof \MongoDB\Driver\Session) {
     ...
     * @internal
     */
    public static function extract_read_preference_from_options(array $options): ?\MongoDB\Driver\ReadPreference
    {
        if (isset($options['readPreference']) && $options['readPreference'] instanceof \MongoDB\Driver\ReadPreference) {
     ...
     * @internal
     */
    public static function select_server(\MongoDB\Driver\Manager $manager, array $options): \MongoDB\Driver\Server
    {
        $session = self::extract_session_from_options($options);
        $server = $session instanceof \MongoDB\Driver\Session ? $session->getServer() : null;
        ...
        $readPreference = self::extract_read_preference_from_options($options);

        // Read preference for an active transaction takes priority
        if ($readPreference === null && $session instanceof \MongoDB\Driver\Session && $session->isInTransaction()) {
            /* Session::getTransactionOptions() should always return an array if the
             * session is in a transaction, but we can be defensive. */
            $readPreference = self::extract_read_preference_from_options($session->getTransactionOptions() ?? []);
        }
     ...
     */
    public static function select_server_for_aggregate_write_stage(\MongoDB\Driver\Manager $manager, array &$options): \MongoDB\Driver\Server
    {
        $readPreference = self::extract_read_preference_from_options($options);
        ...
        if ($readPreference === null || $readPreference->getModeString() === \MongoDB\Driver\ReadPreference::PRIMARY) {
            return self::select_server($manager, $options);
        }
        ...
        try {
            $server = self::select_server($manager, $options);
        } catch (\MongoDB\Driver\Exception\RuntimeException $serverSelectionError) {
        }
        ...
        if (!self::all_servers_support_write_stage_on_secondary($manager->getServers())) {
            $options['readPreference'] = new \MongoDB\Driver\ReadPreference(\MongoDB\Driver\ReadPreference::PRIMARY);
     ...
     * @internal
     */
    public static function select_server_for_write(\MongoDB\Driver\Manager $manager, array $options): \MongoDB\Driver\Server
    {
        return self::select_server($manager, $options + ['readPreference' => new ReadPreference(ReadPreference::PRIMARY)]);
    }
}